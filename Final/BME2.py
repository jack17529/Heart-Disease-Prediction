...
dataset = np.genfromtxt('processed.cleveland.data.txt',dtype = float, delimiter=',')
#print dataset
...
def plot_2D(data,target,target_names):
    ...
    for i,c, label in zip(target_ids,colors,target_names):
        plt.scatter(data[target==i,8],data[target==i,1],c=c,label=label)
    plt.legend()
...
X_train,X_test,y_train,y_test = cross_validation.train_test_split(X_new,y,test_size=0.0,random_state=0)
...
print (modelSVM.score(X_test1,y_test1))
